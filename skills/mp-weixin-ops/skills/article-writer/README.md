...
- 禁止在同一篇文章中混用不同 --style 参数值，除非文章明确跨越多个领域。所有配图应共享同一 --style 和色彩基调。
...
python3 ${SKILL_DIR}/../image-generator/scripts/generate_image.py \
  "YOUR 8-DIMENSION PROMPT, no text, no watermark" \
  --style tech \
  --size 1536x1024 \
  --api-key sk-xxx \
...
**尺寸规则：** 文章插图始终使用横图方向，默认 `1536x1024`（3:2）。禁止使用竖图尺寸如 `1024x1536`。
