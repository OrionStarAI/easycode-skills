...
| 分类 | 适用范围 |
| --- | --- |
| `文档办公` | Word、Excel、PDF、协作文档等办公处理 |
| `演示与设计` | PPT、视觉设计、图表、画布和演示素材 |
| `视频创作` | 视频生成、动画、渲染、演讲视频和后期制作 |
| `社媒运营` | 公众号、抖音及其他社交媒体发布运营 |
| `开发工具` | 编程、MCP、环境安装等开发辅助 |
| `浏览器与测试` | 浏览器自动化、网页产物和测试 |
| `阅读与知识` | 阅读、检索、资讯聚合和知识整理 |

不要使用 `视频与音频` 这类混合分类；若将来出现纯音频能力，再新增独立的 `音频处理` 分类并同步更新门禁规则。仓库门禁会拒绝未登记的分类，避免同义分类长期漂移。
